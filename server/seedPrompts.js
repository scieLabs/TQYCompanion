...
// app.use(express.json());
  ...
  const createPrompt = async (req, res) => {
    try {
        await Prompt.deleteMany({}); // Clear existing users
        const newPrompts = await Prompt.insertMany(prompts); // Seed with new users
        // res.json(newPrompts);
        process.exit();

    } catch (error) {
        console.log(error, 'Error seeding prompts');
        process.exit(1);
    }
